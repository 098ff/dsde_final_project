...
VERIFICATION_CSV_PATH = Path(__file__).parent.parent.parent / "output_data" / "verification_report.csv"
RESOLUTION_CSV_PATH = Path(__file__).parent.parent.parent / "output_data" / "verification_resolutions.csv"
...
with tab_verify:
    ...
    struct_issues  = len(df_ver[df_ver["issue_type"].isin(["MISSING_FOLDER", "MISSING_CSV", "READ_ERROR"])])
    ...
    if HAS_PLOTLY:
        ...
        st.dataframe(
            type_counts[["label", "count"]].rename(columns={"label": "Issue Type", "count": "Count"}),
            hide_index=True,
        )
    ...
    sel_ver_amphoe = fcol1.multiselect(
        "อำเภอ (Amphoe)",
    ...
    sel_ver_type = fcol2.multiselect(
        "Issue Type",
    ...
    sel_ver_file = fcol3.multiselect(
        "File Type",
    ...
    sel_ver_resolution = fcol4.multiselect(
        "Resolution",
                ...
                for tambon, tam_df in amp_df.groupby("tambon", sort=True):
                    ...
                    for unit, unit_df in tam_df.groupby("unit", sort=True):
                        ...
                        for _idx, (_, issue_row) in enumerate(unit_df.iterrows()):
                            ...
                            issue_label = _ISSUE_LABELS.get(issue_row["issue_type"], issue_row["issue_type"])

                            r_col1, r_col2 = st.columns([0.65, 0.35])
                            with r_col1:
                                res_badge = _RESOLUTION_LABELS.get(current_res, current_res)
                                st.markdown(
                            ...
                            with r_col2:
                                st.selectbox(
                                    "Resolution",
                                    options=_RESOLUTION_OPTIONS,
                                    index=_RESOLUTION_OPTIONS.index(current_res)
